...
DMACU_PREFIX = """
    ...
    /* Include the PL080 DMA processing library */
...
def parseCmdLine():
    ...
    parser.add_argument('--input',  required=True, type=str,
                        help='Input assembly file produced by movfuscator')
    parser.add_argument('--output', required=True, type=str,
                        help='Output assembly file produced by movme-dma')
...
def main():
    ...
    if (transpiler.parser_errors > 0):
        logging.error("%u parser error(s) detected while processing the input file.",
                      transpiler.parser_errors)
        sys.exit(-1)
